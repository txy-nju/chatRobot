...
Core orchestration engine. Receives incoming messages, loads the active skill, builds conversation context using a sliding window, calls the LLM, and sends replies through the platform adapter. Handles trigger mode, reply length truncation, and keyword blacklist filtering.
