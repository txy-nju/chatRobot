...
Message platform abstraction layer. Defines a unified message model and adapter interface so the bot engine can interact with any messaging platform (Feishu, Discord, Slack, etc.) through the same API. First implementation targets Feishu (Lark).
